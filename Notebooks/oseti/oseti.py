...
class Analyzer(object):
    ...
    def _calc_sentiment_polarity(self, sentence):
        ...
        #print(morph_analysis(sentence))
        node = morph_analysis(sentence)
        #print(type(node))
        #print(f"node {node}")
        #while node:
        for i, token in enumerate(node): #better enumerate
            ...
            if token:
                ...
                if lemma in self.word_dict:
                    ...
                    if i <len(token)-1 and len(node)>1: # adjust that the sentence contains more than one token
                    #n_parallel += node.next.surface in PARELLEL_PARTICLES
                        n_parallel += node[i+1] in PARELLEL_PARTICLES
                else:
